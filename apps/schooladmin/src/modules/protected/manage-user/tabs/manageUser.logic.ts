...
export const useManageUserLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<ManageUserFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<ManageUserFilters>({
    first_name: "",
    role: "",
    status: "",
  });
  ...
  const applyFilters = (values: ManageUserFilters) => {
