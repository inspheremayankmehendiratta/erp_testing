...
import { OverallSummaryFilters } from "./overallSummary.types";

export const useOverallSummaryLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<OverallSummaryFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<OverallSummaryFilters>({
    state: '',
    name: '',
  });
  ...
  const applyFilters = (values: OverallSummaryFilters) => {
    setFilterValues(values);
    setIsFilterApplied(true);
    updateFilters(removeEmpty(values));
  };

  const resetFilters = (values: OverallSummaryFilters) => {
    setFilterValues(values);
