...
export const useDailyAttendanceSheetLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<DailyAttendanceSheetFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<DailyAttendanceSheetFilters>({
    state: '',
    name: '',
  });
  ...
  const applyFilters = (values: DailyAttendanceSheetFilters) => {
    setFilterValues(values);
    setIsFilterApplied(true);
    updateFilters(removeEmpty(values));
  };

  const resetFilters = (values: DailyAttendanceSheetFilters) => {
    setFilterValues(values);
