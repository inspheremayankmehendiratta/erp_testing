...
export const MANAGE_CONSOLIDATED = `${MANAGE_USER}`;

export const useConsolidatedReportLogic = () => {
  const { data: session } = useSession();
  const fileLabel = 'consolidated_report';
  const visibleColumnsCount = 10;
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<ConsolidatedFilters>({
    url: MANAGE_CONSOLIDATED,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<ConsolidatedFilters>({
    class_section: '',
  });
  ...
  const applyFilters = (values: ConsolidatedFilters) => {
