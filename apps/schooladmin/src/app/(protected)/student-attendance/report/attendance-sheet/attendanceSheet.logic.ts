...
export const useAttendanceSheetLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<AttendanceSheetFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<AttendanceSheetFilters>({
    state: '',
    name: '',
  });
  ...
  const applyFilters = (values: AttendanceSheetFilters) => {
    setFilterValues(values);
    setIsFilterApplied(true);
    updateFilters(removeEmpty(values));
  };

  const resetFilters = (values: AttendanceSheetFilters) => {
    setFilterValues(values);
