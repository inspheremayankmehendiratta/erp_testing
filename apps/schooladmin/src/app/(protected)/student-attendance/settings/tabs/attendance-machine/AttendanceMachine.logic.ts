...
import { AttendanceMachineFilters } from './AttendanceMachine.types';


// placeholder API constant; replace with real endpoint when available
export const MANAGE_ATTENDANCE_MACHINES = `${MANAGE_USER}`;

export const useAttendanceMachineLogic = () => {
  const { data: session } = useSession();
  const fileLabel = 'attendance_machine';
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<AttendanceMachineFilters>({
    url: MANAGE_ATTENDANCE_MACHINES,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] =
    useState<AttendanceMachineFilters>({
      first_name: '',
      email: '',
      mobile: '',
      role: '',
  ...
  const applyFilters = (values: AttendanceMachineFilters) => {
