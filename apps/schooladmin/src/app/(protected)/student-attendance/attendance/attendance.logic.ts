...
export const useAttendanceLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<AttendanceFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<AttendanceFilters>({
    state: '',
    name: '',
  });
  ...
  const applyFilters = (values: AttendanceFilters) => {
    setFilterValues(values);
    setIsFilterApplied(true);
    updateFilters(removeEmpty(values));
  };

  const resetFilters = (values: AttendanceFilters) => {
    setFilterValues(values);
