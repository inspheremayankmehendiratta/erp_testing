...
import { FieldWiseModifyFilters } from "./FieldWiseModify.types";

export const useFieldWiseModifyLogic = () => {
  ...
  const {
    ...
    changeLimit,
    exportData,
  } = useTableData<FieldWiseModifyFilters>({
    url: MANAGE_USER,
    initialFilters: {},
    fileName: fileLabel,
  ...
  const [filterValues, setFilterValues] = useState<FieldWiseModifyFilters>({
    state: '',
    name: '',
  });
  ...
  const applyFilters = (values: FieldWiseModifyFilters) => {
    setFilterValues(values);
    setIsFilterApplied(true);
    updateFilters(removeEmpty(values));
  };

  const resetFilters = (values: FieldWiseModifyFilters) => {
    setFilterValues(values);
